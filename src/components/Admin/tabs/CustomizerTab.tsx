...
export const CustomizerTab: React.FC = () => {
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      {/* Sub-Navigation Tabs */}
      <div className="flex flex-wrap gap-2 p-1.5 bg-bg-card border border-border-color rounded-2xl shadow-sm">
        {[
            ...
            {PRESET_PALETTES.map((preset, idx) => {
              ...
              return (
                <div
                  ...
                  className={`p-5 rounded-2xl border cursor-pointer transition-all duration-200 relative overflow-hidden group shadow-sm ${
                    isSelected
                      ? 'bg-bg-card border-rose-500 ring-2 ring-rose-500/30 shadow-xl shadow-rose-500/10'
                      : 'bg-bg-card border-border-color hover:border-border-hover hover:bg-bg-card-hover'
                  }`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm font-bold text-text-primary group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors">
                      {preset.name}
                    </span>
                    {isSelected && (
                  ...
                  {/* Swatches preview */}
                  <div className="flex items-center gap-2 mt-4 p-3 rounded-xl bg-bg-secondary border border-border-color">
                    <div
                      className="w-7 h-7 rounded-lg shadow-inner border border-black/10 dark:border-white/10"
      ...
      {activeSubSection === 'colors' && (
        <div className="bg-bg-card border border-border-color rounded-2xl p-6 space-y-6 shadow-sm">
          <div>
            <h3 className="text-base font-bold text-text-primary">Custom Color Tuning</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Primary Accent Color</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
                  value={currentTheme.primaryAccent || '#A50C18'}
                  onChange={(e) => handleColorChange('primaryAccent', e.target.value)}
                  className="w-10 h-10 rounded-lg cursor-pointer bg-transparent border border-border-color"
                />
                <input
                  type="text"
                  value={currentTheme.primaryAccent || ''}
                  onChange={(e) => handleColorChange('primaryAccent', e.target.value)}
                  className="flex-1 bg-bg-secondary border border-border-color rounded-xl px-3 py-2 text-sm text-text-primary font-mono focus:border-rose-500 focus:outline-none transition-colors"
                />
              </div>
            </div>

            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Secondary Accent / Hover</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Dark Accent (Shadow / Gradient)</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Canvas / Body Background</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Card & Container Background</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Text Primary Color</label>
              <div className="flex items-center gap-3">
                <input
                  type="color"
      ...
      {activeSubSection === 'typography' && (
        <div className="bg-bg-card border border-border-color rounded-2xl p-6 space-y-6 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Display & Heading Font</label>
              <select
                value={currentTheme.fontHeading || 'Plus Jakarta Sans'}
                onChange={(e) => handleColorChange('fontHeading', e.target.value)}
                className="w-full bg-bg-secondary border border-border-color rounded-xl px-3 py-2.5 text-sm text-text-primary focus:border-rose-500 focus:outline-none transition-colors"
              >
                {HEADING_FONTS.map((font) => (
                  <option key={font} value={font} className="bg-bg-card text-text-primary">
                    {font}
                  </option>
                ))}
              </select>
              <p className="text-xs text-text-muted mt-1.5">Applied to main section titles, hero headings, and cards.</p>
            </div>

            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Body & Paragraph Font</label>
              <select
              ...
              <p className="text-xs text-text-muted mt-1.5">Applied to navigation, body copy, and form inputs.</p>
            </div>
          </div>

          <div>
            <label className="block text-xs font-semibold text-text-secondary mb-3">Card & Button Corner Style (Border Radius)</label>
                  ...
                  <button
                    ...
                    className={`p-3 rounded-xl border text-xs font-semibold transition-all ${
                      isSelected
                        ...
                        : 'bg-bg-secondary text-text-muted border-border-color hover:text-text-primary hover:border-border-hover'
                    }`}
                  >
                    {r.label}
                  </button>
      ...
      {activeSubSection === 'branding' && (
        <div className="bg-bg-card border border-border-color rounded-2xl p-6 space-y-6 shadow-sm">
          <h3 className="text-base font-bold text-text-primary mb-2">Personal & Agency Branding</h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Display Name / Site Name</label>
              <input
                type="text"
                value={currentBranding.siteName || ''}
                onChange={(e) => handleBrandingChange('siteName', e.target.value)}
                className="w-full bg-bg-secondary border border-border-color rounded-xl px-3 py-2 text-sm text-text-primary focus:border-rose-500 focus:outline-none transition-colors"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Tagline / Professional Title</label>
              <input
                type="text"
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-semibold text-text-secondary mb-2">Short Bio</label>
              <textarea
                rows={3}
                value={currentBranding.bio || ''}
                onChange={(e) => handleBrandingChange('bio', e.target.value)}
                className="w-full bg-bg-secondary border border-border-color rounded-xl px-3 py-2 text-sm text-text-primary focus:border-rose-500 focus:outline-none resize-none transition-colors"
              />
            </div>

            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Contact Email</label>
              <input
                type="email"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Contact Phone</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Location</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Avatar / Portrait URL</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">WhatsApp Direct Link</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-text-secondary mb-2">Resume PDF URL</label>
              <input
                type="text"
            ...
            <div className="flex items-center gap-3 pt-4">
              <input
                ...
                className="w-4 h-4 text-rose-600 rounded bg-bg-secondary border-border-color focus:ring-rose-500"
              />
              <label htmlFor="availableForWork" className="text-sm font-semibold text-text-primary cursor-pointer">
